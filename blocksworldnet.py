...
pn.add_arc(ArcPlaceToTransition(pred_clear, act_pickup, var_ob))          # delete
pn.add_arc(ArcPlaceToTransition(pred_on_table, act_pickup, var_ob))       # delete
pn.add_arc(ArcPlaceToTransition(pred_arm_empty, act_pickup, dotLiteral))  # delete
pn.add_arc(ArcTransitionToPlace(act_pickup, pred_holding, var_ob))        # add

# Putdown
act_putdown = pn.add_transition(Transition("act_putdown"))
pn.add_arc(ArcTransitionToPlace(act_putdown, pred_clear, var_ob))          # add
pn.add_arc(ArcTransitionToPlace(act_putdown, pred_on_table, var_ob))       # add
pn.add_arc(ArcTransitionToPlace(act_putdown, pred_arm_empty, dotLiteral))  # add
pn.add_arc(ArcPlaceToTransition(pred_holding, act_putdown, var_ob))        # delete

# Stack
act_stack = pn.add_transition(Transition("act_stack"))
pn.add_arc(ArcPlaceToTransition(pred_clear, act_stack, var_ob))          # require pred_clear
pn.add_arc(ArcTransitionToPlace(act_stack, pred_clear, var_ob))
pn.add_arc(ArcTransitionToPlace(act_stack, pred_arm_empty, dotLiteral))  # add pred_arm_empty
pn.add_arc(ArcPlaceToTransition(pred_holding, act_stack, var_ob))        # delete pred_holding
pn.add_arc(ArcTransitionToPlace(act_stack, pred_on, ProductColorValue(paramColors2, [var_ob, var_underob])))  # add pred_arm_empty

# Unstack
act_unstack = pn.add_transition(Transition("act_unstack"))
pn.add_arc(ArcTransitionToPlace(act_unstack, pred_clear, var_ob))          # require pred_clear
pn.add_arc(ArcPlaceToTransition(pred_clear, act_unstack, var_ob))
pn.add_arc(ArcPlaceToTransition(pred_arm_empty, act_unstack, dotLiteral))  # delete pred_arm_empty
pn.add_arc(ArcTransitionToPlace(act_unstack, pred_holding, var_ob))        # add pred_holding
pn.add_arc(ArcPlaceToTransition(pred_on, act_unstack, ProductColorValue(paramColors2, [var_ob, var_underob])))  # del pred_arm_empty
